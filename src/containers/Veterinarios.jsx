...
const Veterinarios = () => {
	...
	const Content = () => {
		...
		listVets = data?.getAllVets.map((veterinarios, i) => {
			return (
				<InfoMostrada key={i} method={() => desplegarMenu(veterinarios._id)}>
		...
		const MenuCompleto = () => {
			let { fullname, clinic, id, email, phone } = data?.getAllVets.filter(
				(el) => el._id === focusId
			)[0];

			const [deleteVetById, { data: deleteVet }] =
				useMutation(DELETE_VET_BY_ID);
